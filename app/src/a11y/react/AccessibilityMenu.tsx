...
import { useFontSize, useAccessibilityFont } from "./accessibilityPreferences";
...
export default function AccessibilityMenu({
  ...
  const [reduceMotion, setReduceMotion] = useState<boolean>(() => {
    /* v8 ignore next -- SSR guard, window always exists in a browser/jsdom test */
  ...
  useEffect(() => {
    /* v8 ignore next -- mounted/document are always true in a browser/jsdom test (SSR guard) */
    if (mounted && typeof document !== "undefined") {
      document.documentElement.classList.toggle(
        "high-contrast",
  ...
  const toggleReduceMotion = () => {
    ...
    /* v8 ignore next -- SSR guard, document always exists in a browser/jsdom test */
  ...
  /* v8 ignore next 3 -- SSR-only branch: mounted is already true on the first client render in tests */
  if (!mounted) {
  ...
  const getColorVisionLabel = (mode: string): string => {
    ...
    /* v8 ignore next -- defensive fallback, every real mode already has a map entry */
  ...
  /* v8 ignore start -- react-select's internal keyboard handling isn't reliably
	   triggerable through jsdom + user-event; covered by manual/E2E testing instead */
      ...
      <div className="accessibility-menu__category">
        ...
        <div className="accessibility-menu__visual-help-group">
          ...
          <Select
            ...
            onChange={(option) => {
              /* v8 ignore next -- this select isn't clearable, react-select never calls back with null */
            ...
            isSearchable={false}
            /* v8 ignore next 3 -- SSR guard, document always exists in a browser/jsdom test */
            menuPortalTarget={
