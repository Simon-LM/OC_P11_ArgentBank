...
each a _different_ rail weight, by design, so a surface that should
...
3. **Anti-FOUC** — set `data-theme` on `<html>` **before first
   ...
   **Static HTML / Vite / any non-SSR SPA** — there's no server render
   ...
   - **Precompute once** (simplest): run the function once at build time
     ...
     `<head>`. It only changes if your theme _list_ changes, so it's
     safe to hardcode:
       ...
       plugins: [
         {
           ...
           transformIndexHtml: (html) =>
             html.replace(
               "</head>",
               `<script>${themeInitScript(THEMES)}</script></head>`,
             ),
         },
       ],
     };
...
```scss
@use "darkmode-plus-a11y/scss/modules/a11y-fonts" as * with (
  $a11y-fonts-path: "/fonts"
);
