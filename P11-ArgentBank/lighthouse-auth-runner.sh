...
function inject_auth {
  ...
  node lighthouse-storage-injector.js "${BASE_URL}/${page}"
}
...
function run_lighthouse {
  ...
  local output_file="${REPORTS_DIR}/${page}-${device}.html"
  ...
  node lighthouse-runner.js --url "$url" $device_flag --output-path "$output_file"
