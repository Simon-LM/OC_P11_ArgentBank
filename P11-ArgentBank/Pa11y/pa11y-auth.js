...
export default async (page) => {
	...
	try {
		...
		const screenshotPath = path.join(
			__dirname,
			"screenshots/debug/debug_before_submit_click.png"
		);
		console.log(
		...
		if (!currentUrl.includes("/user")) {
			console.error(
				`<<<<< PA11Y AUTH SCRIPT: ERROR - Expected to be on /user page, but on ${currentUrl} >>>>>`
			);
			const errorScreenshotPath = path.join(
				__dirname,
				"screenshots/errors/error_after_login_wrong_page.png"
			);
			...
			throw new Error(
				`Authentication failed. Expected to navigate to /user, but landed on ${currentUrl}`
			);
		}
		...
		try {
			...
			const verificationErrorScreenshotPath = path.join(
				__dirname,
				"screenshots/errors/error_auth_user_page_content_verification_failed.png" // Nom de fichier de capture d'écran spécifique
			);
			try {
		...
		const errorScreenshotPath = path.join(
			__dirname,
			"screenshots/errors/error_in_auth_script.png"
		);
		try {
