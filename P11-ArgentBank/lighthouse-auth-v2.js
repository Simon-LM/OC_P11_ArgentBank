 ...
 */
async function saveCookies(authData, filePath = "./auth-cookies.json") {
	try {
		await fs.writeFile(filePath, JSON.stringify(authData, null, 2));
		console.log(`💾 Données d'authentification sauvegardées dans: ${filePath}`);
	} catch (error) {
		console.error(
...
async function loadCookies(filePath = "./auth-cookies.json") {
	try {
		...
		console.log(`📂 Données d'authentification chargées depuis: ${filePath}`);
